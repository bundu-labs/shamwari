...
title: Architecture
summary: >-
  Two services, drawn along one line: the edge holds no database credentials,
  and the service that holds them does not run on the edge.
order: 4
---
...
1. **Through the AI gateway** — which adds caching, spend limits and
   observability.
2. **Directly to the same provider** — with no Cloudflare in the path at all.
3. **To a smaller model** — lower quality, still an answer.
